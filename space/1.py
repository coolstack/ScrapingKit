...
def get_teams(league):
    ...
    if league == "ncf":
        ...
        if league == "wnba":
            ...
            selector = "div.mt3"
        regions = soup.select('div.mt7')
        for region in regions:
        	state = ''
        	for var in region.select('div.headline') :
        		state = var.text
	        team_divs = region.select(selector)
	        for team_div in team_divs:
	        	teams.append({'state':state, 'id': team_div.find("a")['href'].split('/')[-2], 'name': team_div.find("h2").text})
    return teams

def get_player_id(linkPlain):
	pos = linkPlain.find('/player/_/id/')
	if pos == -1 :
		return -1
	nxt = linkPlain[pos+13:]
	posSlash = nxt.find('/')
	id = nxt[:posSlash]
	return id

def get_player_stat(league,id):
	url = 'https://www.espn.com/'+league+'/player/stats/_/id/'+str(id)
	print(url)
	soup = get_soup(retry_request(url))
	divSoup = soup.select("div.ResponsiveTable.pt4")

	divLen = len(divSoup)
	ret = []
	for k in range(divLen) :
		div = divSoup[k]
		titleSoup = div.select("div.Table__Title")
		title = titleSoup[0].text
		tableSoup = div.select("table.Table")

		# Season scrape
		trSoup = tableSoup[0].select("tr.Table__TR")
		n = len(trSoup)
		seasonList = []
		career = []
		for i in range(1,n-1):
			tr = trSoup[i]
			tdSoup = tr.select("td.Table__TD")
			seasonList.append({'season':tdSoup[0].text,'team':tdSoup[1].text})
		#Stat scrape
		trSoup = tableSoup[1].select('tr.Table__TR')
		n = len(trSoup)
		thList = []
		for i in range(n) :
			tr = trSoup[i]
			if i == 0 : # Table Head
				thSoup = tr.select("th.Table__TH")
				for th in thSoup:
				...
				if i == n-1 :
					...
					seasonList[i-1]['stat'] = stat
		print(seasonList)
		print(career)
		print(thList)
		ret.append({'seasonList':seasonList,'career':career,'thList':thList})
	return ret

def get_team_info(league,team_id):
	url = BASE_URL + "/" + league + "/team/stats/_/name/"+team_id
	soup = get_soup(retry_request(url))
	leadersSoup = soup.select("h3.di")
	leaders = []
	for leader in leadersSoup :
		leaders.append(leader.contents[0])

	url = BASE_URL + "/" + league + "/team/roster/_/name/"+team_id
	soup = get_soup(retry_request(url))
	tableSoup = soup.select("tr.Table__TR")
	members = []
	for tr in tableSoup :
		step = 0
		divSoup = tr.select("div.inline")
		player = {}
		done = False
		for div in divSoup :
			tot = str(div)
			...
			if '--headshot' in tot:
				continue
			if "player/_/id/" in tot:
				done = True
				player['id'] = get_player_id(tot)
				player['name'] = div.contents[0].text
			if step == 3 :
				player['pos'] = div.text
		if done == True :
			members.append(player)
	injury_url = 'https://www.espn.com/'+league+'/team/injuries/_/name/'+team_id
	soup = get_soup(retry_request(injury_url))
	injuriesSoup = soup.select("div.ContentList__Item")
	injuries = []
	for var in injuriesSoup :
		id = get_player_id(str(var))
		if id == -1 :
			continue
		injuries.append(id)
	print(leaders)
...
def get_fighter(id):
	...
	print(url)
	soup = get_soup(retry_request(url))
	bioSoup = soup.select('div.PlayerHeader__Bio')
	player = {}
	for bio in bioSoup:
		liSoup = bio.select('li')
		for li in liSoup :
			ttuSoup = li.select('div.ttu')
			for ttu in ttuSoup:
				player[ttu.text] = li.text[len(ttu.text):]
	nameSoup = soup.select('span.truncate.min-w-0')
	fullName = ''
	...
	player['id'] = id
	statSoup = soup.select('div.StatBlockInner__Value')
	step = 0
	statIns = ['W-L-D', '(T)KO', 'SUB']
	for stat in statSoup :
		player[statIns[step]] = stat.text
		step += 1
	return player
...
def get_fighter_list():
	...
	for alpha in alphaList :
		url = 'http://www.espn.com/mma/fighters?search=' + alpha
		soup = get_soup(retry_request(url))
		hrefSoup = soup.select('a')
		for href in hrefSoup :
			tot = str(href)
			key = '<a href="/mma/fighter/_/id/'
			pos = tot.find(key)
			if pos == -1 :
				continue
			nxt = tot[pos+len(key):]
...
stats_regex = ('\[\"Career\",\"\",(.*?)\]\},\{\"ttl\"\:\"Regular Season Misc Totals\"')
